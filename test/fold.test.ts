    ...
    describe('foldPath', () => {
      it('renders a deterministic tree and per-file sections', async () => {
        ...
        expect(result.files.map((file) => file.relativePath)).toEqual(['README.md', 'src/index.ts', 'src/util.ts']);
      ...
      it('respects ignore files and sensitive path protection', async () => {
        ...
        expect(result.files.find((file) => file.relativePath === 'ignored.log')).toBeUndefined();
        expect(result.files.find((file) => file.relativePath === '.env')).toBeUndefined();
      ...
      it('enforces per-file and aggregate budgets', async () => {
        ...
        expect(fileLimited.skipped).toEqual(
          expect.arrayContaining([{ relativePath: 'src/index.ts', reason: 'file-too-large' }])
        );
        ...
        expect(budgetLimited.skipped.some((entry) => entry.reason === 'budget-exceeded')).toBe(true);
      });
      ...
      it('skips symlinks and binary files', async () => {
        ...
        const externalRoot = await mkdtemp(path.join(os.tmpdir(), 'sourcefold-external-'));
...
', 'utf8');
        ...
        await writeFile(path.join(fixture, 'binary.bin'), Buffer.from([0, 1, 2, 3]));
